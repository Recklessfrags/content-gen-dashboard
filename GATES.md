...
| 6 | Deploys clean on Vercel from a fresh clone; no secrets in repo; Supabase keys in env | **PENDING** | Build clean; no secrets tracked (only `.env.example`); env documented. **Deploy not executed (P-1).** |
